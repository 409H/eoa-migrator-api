<?php
namespace App\Http\Providers\Transactions\Polygon;
...
class Consumer extends BaseProvider implements ProviderInterface 
{
    ...
    const PROVIDER_NAME = "Polygonscan";
    ...
    public function getDataFromApi(string $userAddress): Consumer
    {
        ...
        $client = new Client([
            'base_uri' => 'https://api.polygonscan.com/api',
            'headers' => [
            ],
        ]);
        ...
        try {
            $res = $client->request(
                'GET', 
                "?module=account&action=txlist&address=". $this->userAddress ."&startblock=0&endblock=99999999&page=1&offset=0&sort=asc&apikey=". env("PROVIDER_TX_POLYGONSCAN_API_KEY", "NO_API_KEY")
            );
