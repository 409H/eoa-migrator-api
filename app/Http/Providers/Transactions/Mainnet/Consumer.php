...
class Consumer extends BaseProvider implements ProviderInterface 
{
    ...
    public function getDataFromApi(string $userAddress): Consumer
    {
        ...
        try {
            $res = $client->request(
                'GET', 
                "?module=account&action=txlist&address=". $this->userAddress ."&startblock=0&endblock=99999999&page=1&offset=0&sort=asc&apikey=". env("PROVIDER_TX_ETHERSCAN_API_KEY", "NO_API_KEY")
            );
