<?php
namespace App\Http\Providers\Transactions\Kovan;
...
{
    const SUPPORTED_NETWORK_ID = 42;
    const PROVIDER_NAME = "Kovan (Etherscan)";
    ...
    public function getDataFromApi(string $userAddress): Consumer
    {
        ...
        $client = new Client([
            'base_uri' => 'https://api-kovan.etherscan.io/api',
            'headers' => [
            ],
        ]);
        ...
        try {
            $res = $client->request(
                'GET', 
                "?module=account&action=txlist&address=". $this->userAddress ."&startblock=0&endblock=99999999&page=1&offset=0&sort=asc&apikey=". env("PROVIDER_TX_KOVAN_API_KEY", "NO_API_KEY")
            );
