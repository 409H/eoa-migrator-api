<?php
namespace App\Http\Providers\Transactions\Rinkeby;
...
class Consumer extends BaseProvider implements ProviderInterface 
{
    ...
    const PROVIDER_NAME = "Rinkeby (Etherscan)";
    ...
    public function getDataFromApi(string $userAddress): Consumer
    {
        ...
        $client = new Client([
            'base_uri' => 'https://api-rinkeby.etherscan.io/api',
            'headers' => [
            ],
        ]);
        ...
        try {
            $res = $client->request(
                'GET', 
                "?module=account&action=txlist&address=". $this->userAddress ."&startblock=0&endblock=99999999&page=1&offset=0&sort=asc&apikey=". env("PROVIDER_TX_RINKEBY_API_KEY", "NO_API_KEY")
            );
